...
def test_category_delete(api_client_admin, category1):
    url = reverse('task_update_detail_remove_api', kwargs={'pk': category1.id})
    client, _ = api_client_admin
...
@pytest.mark.django_db
def test_product_category1_list(api_client_auth, products_category1):
...
@pytest.mark.django_db
def test_cart_item_unauth_update(api_client_auth, cart_item_unauth, product, cart):
    url = reverse('item_update_detail_remove_api', kwargs={'pk': cart_item_unauth.id})
    client, _ = api_client_auth
...
def test_order_update(api_client_auth, order, shipping_address2):
    ...
    client, _ = api_client_auth
    payload = {"shipping_address": shipping_address2}
...
@pytest.mark.django_db
def test_order_payment(api_client_auth):
    url = reverse('order_payment_api')
    client, _ = api_client_auth
...
@pytest.mark.django_db
def test_payment_details_detail(api_client_unauth, payment_details):
    url = reverse('payment_details_update_detail_remove_api', kwargs={'pk': payment_details.id})
    client, _ = api_client_unauth
    response = client.get(url)
    assert response.status_code == 200
...
@pytest.mark.django_db
def test_shipping_address_detail(api_client_unauth, shipping_address):
    url = reverse('shipping_address_update_detail_remove_api', kwargs={'pk': shipping_address.id})
    client, _ = api_client_unauth
    response = client.get(url)
    assert response.status_code == 200
